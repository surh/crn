...
ggsave("syncom_od_plots.png", width = 6, height = 6)
...
p1 <- bind_rows(Syncoms %>%
            select(Community, temp, OD600, total_time_h, batch) %>%
            mutate(type = "Observed"),
          Expected_od %>%
            select(Community, temp, OD600 = OD600_mean, total_time_h) %>%
            mutate(batch = "Predicted_mean") %>%
            mutate(type = "E[Resource competition]"),
          Expected_od %>%
            select(Community, temp, OD600 = OD600_sum, total_time_h) %>%
            mutate(batch = "Predicted_sum") %>%
            mutate(type = "E[No interaction]")) %>%
  filter(total_time_h != 4 & total_time_h != 20) %>%
  mutate(Community = factor(Community, levels = paste0("R", 1:12))) %>%
  mutate(temp = as.character(temp)) %>%
  ggplot(aes(x = total_time_h, y = OD600, 
             group = interaction(Community, temp, batch, sep = "_"))) +
  facet_wrap(~ Community) +
  geom_vline(xintercept = c(24, 48)) +
  geom_line(aes(col = temp, linetype = type)) +
  ...
  theme(legend.position = "bottom",
        legend.box = "vertical")
p1
ggsave("syncom_od_plots_preds.svg", width = 6, height = 6)
ggsave("syncom_od_plots_preds.png", width = 6, height = 6)
